...
public class P_ControllerEventListener : MonoBehaviour {
	...
	void Start () {
		...
		// Adds all event handlers for actions only the right controller should do
		...
		// Adds all event handlers for actions both controllers should do
		cEvents.ButtonOnePressed += new ControllerInteractionEventHandler(TakeScreenshot);
	...
	void TakeScreenshot(object sender, ControllerInteractionEventArgs e){
		pControl.TakeScreenshot();
	}	
}
